...
def _stats(pred: np.ndarray, ref: np.ndarray) -> dict:
    ...
    r = (
        ...
        if pred.size > 2 and np.std(pred) > 0 and np.std(ref) > 0
        else float("nan")
    )
...
def compare(
    ...
    """Metrics for the raw prediction and the scale/offset-aligned prediction (scale invariant)."""
    ...
    out: dict = {"raw": raw, "aligned": aligned}
    if classes:
        out["per_class"] = {}
...
def error_map_png(
    pred: np.ndarray, ref: np.ndarray, path: str | Path, clip: float | None = None
) -> tuple[Path, float]:
    err = pred - ref
    ok = np.isfinite(err)
    if clip is None:
