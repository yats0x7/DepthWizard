...
Monocular depth models predict each tile with its own unknown scale and shift. We run one
low-resolution pass over the whole image, then align every high-resolution tile to that global
map with a least-squares affine fit before blending with a cosine window. The result keeps the
global structure of the low-res pass and the detail of the tiles.
"""
...
def _align(src: np.ndarray, ref: np.ndarray) -> tuple[float, float]:
    """Robust a, b with a*src + b ~= ref (trimmed least squares)."""
    ...
    if not np.isfinite(a) or a <= 0:
        return 1.0, float(np.nanmean(y) - np.nanmean(x))
    return float(a), float(b)


def tiled_predict(infer: Infer, rgb: np.ndarray, tile: int = 1036, overlap: int = 160,
                  global_max: int = 1036, progress: Callable[[float], None] | None = None) -> np.ndarray:
    h, w = rgb.shape[:2]
    scale = min(1.0, global_max / max(h, w))
    small = rgb if scale == 1.0 else cv2.resize(rgb, (round(w * scale), round(h * scale)),
