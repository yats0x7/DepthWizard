"""Heightfield + texture -> textured glTF binary (Y-up, metres when georeferenced)."""
...
def heightfield_to_glb(height: np.ndarray, rgb: np.ndarray, out: str | Path,
                       pixel_size: tuple[float, float] = (1.0, 1.0), max_side: int = 512,
                       z_offset: float | None = None) -> dict:
    h, w = height.shape
    ...
    # X east, Y up, Z south (glTF / three.js convention). Centre the mesh on the origin.
    x = (jj - (mw - 1) / 2) * dx
    zz = (ii - (mh - 1) / 2) * dy
    ...
    tex = rgb
    if max(tex.shape[:2]) > 2048:
        s = 2048 / max(tex.shape[:2])
        tex = cv2.resize(tex, (round(tex.shape[1] * s), round(tex.shape[0] * s)),
    ...
    return {"vertices": int(mesh.vertices.shape[0]), "faces": int(mesh.faces.shape[0]),
            "grid": [mh, mw], "pixel_size": [dx, dy], "z_offset": z_offset}
