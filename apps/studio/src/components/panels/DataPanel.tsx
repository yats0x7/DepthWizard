  ...
  return (
    <>
      <Section title="Your file">
        ...
        <Stat label="Has coordinates" value={m.input.georeferenced ? `yes${m.input.epsg ? `, EPSG:${m.input.epsg}` : ''}` : 'no'} />
        {m.input.pixel_size_m && <Stat label="One pixel covers" value={`${fmt(m.input.pixel_size_m[0], 2)} × ${fmt(m.input.pixel_size_m[1], 2)}`} unit="m" />}
      ...
      <Section title="How it was made">
        <Stat label="Depth model" value={m.model.id.split('/').pop() ?? m.model.id} />
      ...
      <Section title="Calibration">
        ...
        <Stat label="Structure scale" value={fmt(m.calibration.scale, 3)} />
      ...
      <Section title="Height statistics">
        ...
        <Stat label="Typical range" value={fmt(m.stats.relief, unit ? 1 : 3)} unit={unit} />
