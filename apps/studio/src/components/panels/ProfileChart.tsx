        ...
        <AreaChart data={data} margin={{ top: 8, right: 8, left: -18, bottom: 0 }}>
          <defs>
            <linearGradient id="profileFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor="#e5a13c" stopOpacity={0.45} />
              <stop offset="100%" stopColor="#e5a13c" stopOpacity={0.02} />
            </linearGradient>
          </defs>
          <CartesianGrid stroke="#223040" strokeDasharray="2 4" vertical={false} />
          <XAxis dataKey="t" type="number" domain={[0, 'dataMax']} tickFormatter={(v) => fmt(v, 0)} stroke="#857c72" fontSize={10} tickLine={false} axisLine={false} />
          <YAxis domain={[lo - pad, hi + pad]} tickFormatter={(v) => fmt(v, unit === 'm' ? 0 : 2)} stroke="#857c72" fontSize={10} tickLine={false} axisLine={false} width={48} />
          <Tooltip
          ...
          />
          <Area type="monotone" dataKey="h" stroke="#e5a13c" strokeWidth={1.8} fill="url(#profileFill)" isAnimationActive={false} />
        </AreaChart>
      </ResponsiveContainer>
    </div>
  )
